use crate::prompt::{SYSTEM_PROMPT, TRANSFORM_SYSTEM_PROMPT, PROMPT_SYSTEM_PROMPT};
use reqwest::Client;
...
pub async fn optimize_prompt(text: &str, extra: &str, api_key: &str) -> Result<String, String> {
    let client = Client::new();
    let user_content = if extra.is_empty() {
        text.to_string()
    } else {
        format!("{}\n\nAdditional context: {}", text, extra)
    };
    let body = Req {
        model: "openai/gpt-4o-mini".to_string(),
        messages: vec![
            Msg { role: "system".to_string(), content: PROMPT_SYSTEM_PROMPT.to_string() },
